...
export default defineComponent({
  ...
  setup(props, { attrs, emit }) {
    ...
    const getWrapperStyle = computed((): CSSProperties => {
      ...
      if (!raw) return { height: "360px" };
      if (/^\d+$/.test(raw)) return { height: `${raw}px` };
      return { height: raw };
    });
    ...
    function croppered() {
      ...
      const quality = Number.isFinite(props.outputQuality)
        ? Math.min(1, Math.max(0, Number(props.outputQuality)))
        : 0.92;
      const outputType = String(props.outputType || "").trim() || undefined;
