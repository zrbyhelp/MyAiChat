...
const { loginTitle, copyright, getLogo, getDropdownItemStyle, getDropdownItemClass } = useNav();
const { locale, languageOptions, switchLanguage } = useTranslationLang();
const logoLoadError = ref(false);
const loginLogo = computed(() => {
  if (logoLoadError.value) return "/logo.svg";
  return getLogo();
});
      ...
      useUserStoreHook()
        ...
        .catch(err => {
          const errorMessage =
            ...
            err?.message;
          message(
            errorMessage || t("login.pureLoginFail"),
            { type: "error" }
          );
        })
        .finally(() => {
    ...
    <div class="login-container">
      ...
      <div class="login-box">
        <div class="login-form">
          <img :src="loginLogo" class="avatar login-logo" alt="logo" @error="logoLoadError = true" />
          <Motion>
            <h2 class="outline-hidden">
              <TypeIt
                :options="{ strings: [loginTitle], cursor: false, speed: 100 }"
              />
            </h2>
          </Motion>
