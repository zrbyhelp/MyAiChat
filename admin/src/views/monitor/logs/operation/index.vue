...
async function openNotifyDialog() {
  ...
  try {
    const [settingRes, userRes] = await Promise.all([
      getLogSizeNotifySetting({ logType: "operation" }),
      getUserList({})
    ]);
...
<template>
  <div class="main">
    ...
    >
      <el-form-item :label="transformI18n($t('monitor.operationStatus'))" prop="status">
        <el-select
          v-model="form.status"
      ...
      <el-form-item :label="transformI18n($t('monitor.operationTime'))" prop="operatingTime">
        <el-date-picker
    ...
    <PureTableBar :title="transformI18n($t('monitor.operationLog'))" :columns="columns" @refresh="onSearch">
      <template #title>
        <div class="flex items-center gap-3">
          <span class="font-bold truncate">{{ transformI18n($t("monitor.operationLog")) }}</span>
