...
async function openNotifyDialog() {
  ...
  try {
    const [settingRes, userRes] = await Promise.all([
      getLogSizeNotifySetting({ logType: "system" }),
      getUserList({})
    ]);
...
<template>
  <div class="main">
    ...
    >
      <el-form-item :label="transformI18n($t('monitor.keyword'))" prop="keyword">
        <el-input
          v-model="form.keyword"
          :placeholder="transformI18n($t('monitor.enterKeyword'))"
          clearable
          class="w-[220px]!"
        />
      </el-form-item>
      <el-form-item :label="transformI18n($t('monitor.time'))" prop="requestTime">
        <el-date-picker
    ...
    <PureTableBar :title="transformI18n($t('monitor.systemLog'))" :columns="columns" @refresh="onSearch">
      <template #title>
        <div class="flex items-center gap-3">
          <span class="font-bold truncate">{{ transformI18n($t("monitor.systemLog")) }}</span>
        ...
        >
          <template #operation="{ row }">
            <el-button
              ...
              @click="onDetail(row)"
            >
              {{ transformI18n($t("monitor.detail")) }}
            </el-button>
          </template>
        </pure-table>
      </template>
