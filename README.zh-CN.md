...
- 可用的 Clerk 应用
- 若启用 `linux.do` 第三方登录，需要先在 Clerk 中将其配置为外部身份提供方
...
- `CLERK_SECRET_KEY` / `CLERK_PUBLISHABLE_KEY` / `VITE_CLERK_PUBLISHABLE_KEY`：鉴权配置，`linux.do` 第三方登录也复用这组 Clerk 配置
...
3. 保持当前前端“登录”按钮不变，用户仍通过 Clerk 弹窗完成登录，`linux.do` 入口由 Clerk 负责展示。
4. 登录成功后，`chat` 继续使用 Clerk token 访问 `main` 与 `upload`，项目内不新增 OAuth 登录接口。
5. 若 Clerk 弹窗中没有出现 `linux.do`，优先检查 provider 是否启用、回调域名是否匹配、前端是否使用了正确的 Clerk Publishable Key。
